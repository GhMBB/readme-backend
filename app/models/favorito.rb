...
class Favorito < ApplicationRecord
  ...
  def self.buscar_por_usuario_y_libro(libro_id, user_id)
    find_by(libro_id: libro_id, user_id: user_id, favorito: true, deleted: false)
  end
  ...
  def self.libros_favoritos_por_usuario(params)
    ...
    favoritos_query = favoritos_query.joins(libro: :user)
                                     .where("LOWER(libros.titulo) LIKE ? OR LOWER(users.username) LIKE ?", "%#{params[:busqueda].downcase}%", "%#{params[:busqueda].downcase}%") if params[:busqueda].present?
    ...
    return {total_pages: favoritos.total_pages,
